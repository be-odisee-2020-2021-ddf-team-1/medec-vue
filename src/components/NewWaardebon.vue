<template>
  <div id="NewWaardebon" class="container mt-3">
    <b-alert :show="showInfo" variant="dark">
      U krijgt een gratis waardebon van Jaegher. Vul uw gegevens in en Jaegher
      bezorgt u uw waardebon.
    </b-alert>
    ...
    <div id="showForm">
      <form class="w-50" @submit.prevent="submitting" ref="form">
        ...
        <input
          ...
          id="email"
          name="email"
          placeholder="Emailadres@email.be"
          v-model="persoon.email"
        /><br />
        ...
        <input
          ...
          id="adres"
          name="adres"
          placeholder="Straat nummer, Gemeente postcode"
          v-model="persoon.addres"
        /><br />
...
export default {
  ...
  methods: {
    submitting() {
      ...
      axios
        .post(
          "http://localhost:8080/jaegherrest/createklant",
          this.persoon,
          headers
        )
        .then((response) => {
