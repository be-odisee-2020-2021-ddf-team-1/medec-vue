<template>
  <div id="BesteldToestel" class="container mt-3">
    <router-link to="/Create" class="btn btn-primary mb-3" tag="button"
      >New toestel</router-link
    >
    <button class="btn btn-danger mx-3 p-2 float-right" @click="deleteToestel">
      Delete
    </button>
    <button class="btn btn-secondary mx-3 p-2 float-right" @click="toDetails">
      Details
    </button>
...
export default {
  ...
  methods: {
    show() {
      this.url = "http://localhost:8081/besteldtoestel/besteldetoestellen";
      axios
        .get(this.url)
        .then(
          (response) => (
